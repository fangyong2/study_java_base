...
//    static List<Integer> count = new ArrayList<Integer>(); // 借出次数
    ...
    static List<DVDBean> dvdList = new ArrayList<DVDBean>();
    ...
    public void view(){
        ...
        for (int i = 0; i < dvdList.size(); i++) {
            if (dvdList.isEmpty()) {
                break;
            } else if (dvdList.get(i).getState() == 0) { // state[i]为0则说明该DVD已借出
                System.out.println((i + 1) + "\t已借出\t" + "<<"
                        + dvdList.get(i).getName() + ">>\t" + dvdList.get(i).getDate() + "日\t"
                        + dvdList.get(i).getCount() + "次");
            } else if (dvdList.get(i).getState() == 1) { // state[i]为1则说明该DVD可借
