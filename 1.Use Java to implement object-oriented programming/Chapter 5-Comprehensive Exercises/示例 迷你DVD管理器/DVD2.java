...
public class DVD2 {
    /*创建数组存储DVD信息*/
    static String[] names = new String[7]; // 存储DVD名称数组
    static int[] state = new int[7]; // 存储DVD借出状态：0已借出/1可借
    static int[] date = new int[7]; // 存储DVD借出日期
    static int[] count = new int[7]; // 借出次数
	public DVD2() {
        /*初始三个DVD*/
        names[0] = "罗马假日";
        state[0] = 0;
    ...
    public void del(){
        ...
        for (int i = 0; i < names.length; i++) {
            ...
            if (names[i] != null && names[i].equalsIgnoreCase(name)
                    && state[i] == 1) { // equalsIgnoreCase方法判断字符串是否相同且忽略大小写
                int j = i;
